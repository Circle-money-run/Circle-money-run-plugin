...
export class GetMaster extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: `^#跑路(强制)?安装(插件|${Object.keys(pluginList).join("|")})$`,
          fnc: 'install'
        }
      ]
    });
  ...
  async install(e) {
    ...
    const pluginName = e.msg.replace(/^#跑路(强制)?安装/, "").trim();
    if (pluginName === "插件") {
      let msg = "\n";
  const checkPromises = Object.keys(pluginList).map(async (pluginsName) => {  
  try {  
    await fs.promises.access(`plugins/${pluginsName}`);  
    logger.info(`${pluginsName}（${mergedPlugins[pluginsName].ChineseName || ''}）插件存在`);
  } catch (error) {  
    if (error.code === 'ENOENT') {  
      const chineseName = mergedPlugins[pluginsName].ChineseName || ''; 
  ...
  return true; 
      ...
      if (msg === "\n") {
        ...
        msg = `可安装插件列表：${msg}发送 #跑路安装+插件名 进行安装`;
      }
      await e.reply(msg);
      return true;
    }
